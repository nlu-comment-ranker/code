...
python classifier.py $DATA -t $TARGET -c $MODEL -s all_abl --fg all-june
...
python classifier.py $DATA -t $TARGET -c $MODEL -s base_meta_ctxt_abl --fg baseline metadata_only context

python classifier.py $DATA -t $TARGET -c $MODEL -s base_meta_abl --fg baseline metadata_only
